...
rule; make your `ANSWER` pass it before you submit. The model-facing prompt
must apply this check because `Gateway.decide` never receives ANSWER actions.
...
`scan_for_injected_instructions` now catches explicit override,
role-impersonation, secret-exfiltration and cross-learner-write patterns.
Treat that scanner as a deterministic safety signal, while still applying
semantic judgment to paraphrased or novel attacks it cannot recognise.
