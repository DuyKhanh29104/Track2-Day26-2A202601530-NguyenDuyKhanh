...
everything else plugs into it), `strategy.py` (the routing/budget policy
used by the gateway), `guardrails.py` (what your `ANSWER` should
...
| **`guardrails.py`** | Whether your `ANSWER` is grounded, contains injected instructions or common private/credential forms, and whether numeric precision still needs evidence. `abstention_policy` refuses ungrounded answers. | The `answer` L1 event (`text`, `cited_anchors`) the arena hands your opponent's prosecutor on prosecution (CONTRACTS.md section 5.4). | `ungrounded` **5** · `fabricated_citation` **8** · `hallucination` **7** · `guardrail_breach` **8** · `privacy_leak` **8** · `unsupported_precision` **4** · partial credit toward `non_responsive` **4** / `incoherent` **4** via abstaining instead of guessing |
